/* 
  ...
  Correction à faire :
    ...
    -La 1ere citation n'est plus lu.  //OK.
...
let auteurCitation  = document.querySelector('#auteur');
...
//  Fonction qui génere un nombre aléatoire 
//   en donnant le nombre maximum à ne pas dépasser :
function genererNombreEntier(max) {
  return Math.floor(Math.random() * Math.floor(max));
}


//  Creer un évènement sur le bouton au clic :
bouton.addEventListener('click', () => {
  do {
    nombreAleatoire = genererNombreEntier(citations.length);      //  Genere un nb aléatoire. 
  } while (nombreAleatoire == dernier);
  
  citation.textContent        = citations[nombreAleatoire][0];    //  Modifier le contenu de l'id citation.
  auteurCitation.textContent  = citations[nombreAleatoire][1];    //  Modifier le contenu de l'id auteur.
  dernier                     = nombreAleatoire;                  //  On stocke le nb aleatoire dans dernier pour ne pas avoir la meme citation à la suite.
});